...
df_races = spark.table(f"{catalogo}.{esquema_source}.races").withColumnRenamed(
    "name", "name_race")
...
df_circuit = df_circuit.withColumn(
    "altitude_category", altitud_udf("altitude"))

# COMMAND ----------

df_joined = df_races.alias("x").join(df_circuit.alias(
    "y"), col("x.circuit_id") == col("y.circuit_id"), "inner")

# COMMAND ----------

df_filtered_sorted = df_joined.filter(
    df_races.race_year > 1978).orderBy("race_id")
...
df_with_latitude_diff = df_filtered_sorted.withColumn(
    "lat_diff",
    F.abs(df_filtered_sorted["latitude"] -
          df_filtered_sorted["latitude"].alias("latitude")).cast(IntegerType())
)

# COMMAND ----------

df_updated = df_with_latitude_diff.select("*",
                                          when(col("circuit_ref").isin("albert_park", "sepang", "bahrain"), lit(
                                              "Internacional")).otherwise("Local").alias("race_type"),
...
df_updated.write.mode("overwrite").saveAsTable(
    f"{catalogo}.{esquema_sink}.circuits_transformed")
