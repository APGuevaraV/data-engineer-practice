...
newdeptdf_2 = deptdf.select(col("id_2").alias(
    "id_nuevo"), col("name").alias("nombre"))
...
# Inner JOIN.
array = ['id', 'name']
df_join = df.join(deptdf, df["dept"] == deptdf["id_2"], "inner").select(
    [col(column) for column in array])
df_join.display()

# COMMAND ----------

df_join.select(col("id"), col("name"), col("dept"),
               col("salary"), col("nom_dept")).display()
