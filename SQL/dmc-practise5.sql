...
--Procedimiento almacenadao

DECLARE @CODIGO VARCHAR(15),
	    @TOTALPRIMA FLOAT,
		@TOTALSEGUROS FLOAT,
		@VINCULADO VARCHAR(15)
    SET @CODIGO  ='8089664022'
    SET @TOTALPRIMA   = (SELECT SUM(IM_PRIMA) FROM tb_seguro WHERE MES_CORTE=201012 AND CODIGO=@CODIGO)
	SET @TOTALSEGUROS = (SELECT COUNT(*) FROM tb_seguro WHERE MES_CORTE=201012 AND CODIGO=@CODIGO)
	IF(@TOTALSEGUROS>2) 
	BEGIN 
		 SET @VINCULADO='VINCULADO'
	  END
   ELSE
   	BEGIN 
		 SET @VINCULADO='NO VINCULADO'
	  END 
	PRINT 'EL CLIENTE CON CODIGO '+@CODIGO+ ' ES '+@VINCULADO
	PRINT 'ESTE CLIENTE TIENE '  +CONVERT(VARCHAR(10),@TOTALSEGUROS)+' SEGUROS'
	PRINT 'EL TOTAL DE SU PRIMA '+CONVERT(VARCHAR(10),@TOTALPRIMA)  + ' EN SOLES'